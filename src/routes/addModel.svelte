<script>
    ...
    let dockerHubImageExist;
    let model = {};

    $: {
        fetchDockerHubInfoByName(model.dockerHubImageUrl)
            .then((info) => {
                dockerHubImageExist = info.dockerHubImageExist;
            });
    }
    ...
    <form class="flex flex-col" in:fade>
        ...
        <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg width="24px" height="24px" stroke-width="1" viewBox="0 0 24 24" fill="none"
            ...
            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                <svg width="24px" height="24px" stroke-width="1" viewBox="0 0 24 24" fill="none"
                     xmlns="http://www.w3.org/2000/svg" color="#000000">
                    <path d="M7 12.5l3 3 7-7" stroke="#000000" stroke-width="1" stroke-linecap="round"
                          stroke-linejoin="round"></path>
                    <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" stroke="#000000"
                          stroke-width="1" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>
            </div>
            <input type="text" id="name" placeholder="GPT, BART, R-CNN..." bind:value={model.name}
                   class="border-2  rounded-md p-2 pl-10 my-2 w-full">
        </div>
        <label for="image">DockerHub image url (is exist = {dockerHubImageExist})</label>
        <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg width="24px" height="24px" stroke-width="1" viewBox="0 0 24 24" fill="none"
                     xmlns="http://www.w3.org/2000/svg" color="#000000">
                    ...
                    <path d="M12 8c-3.273 0-3.273 2-3.273 3C7.818 11 6 11.5 6 13.5S7.818 16 8.727 16h6.546c.909 0 2.727-.5 2.727-2.5S16.182 11 15.273 11c0-1 0-3-3.273-3z"
                          stroke="#000000" stroke-width="1" stroke-linejoin="round"></path>
                </svg>
            </div>
            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                <svg width="24px" height="24px" stroke-width="1" viewBox="0 0 24 24" fill="none"
            ...
            <input type="text" id="image" placeholder="myrepo/yolo" bind:value={model.dockerHubImageUrl}
                   class="border-2 rounded-md p-2 pl-10 my-2 w-full">
        </div>
        ...
        <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg width="24px" height="24px" stroke-width="1" viewBox="0 0 24 24" fill="none"
                     xmlns="http://www.w3.org/2000/svg" color="#000000">
                    <path d="M8 6h12M4 6.01l.01-.011M4 12.01l.01-.011M4 18.01l.01-.011M8 12h12M8 18h12" stroke="#000000"
                          stroke-width="1" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>
            </div>
